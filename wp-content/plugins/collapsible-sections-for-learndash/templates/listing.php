<?php
/**
 * LearnDash LD30 Displays the listing of course content with collapsible sections
...
if ( ! empty( $lessons ) || ! empty( $quizzes ) ) :
	...
	$table_class .= ' ld-collapsible-sections'; // Add custom class for our functionality
	...
	<div class="<?php echo esc_attr( $table_class ); ?>" id="<?php echo esc_attr( 'ld-item-list-' . $course_id ); ?>" data-ld-expand-id="<?php echo esc_attr( 'ld-item-list-' . $course_id ); ?>" data-ld-expand-list="true">
		...
		if ( $lessons && ! empty( $lessons ) ) :
			...
			/**
			 * Get course sections and organize lessons
			 *
			 * @var $lessons [array]
			 * @since 3.0.0
			 */
			...
			foreach ( $lessons as $lesson ) :
				...
				if ( isset( $sections[ $lesson_id ] ) ) {
					...
					if ( $current_section !== null ) {
						?>
						</div> <!-- Close previous section lessons -->
					...
					<div class="ld-section-wrapper" data-section-id="<?php echo esc_attr( $current_section->ID ); ?>">
						<?php
						// Use custom section template with toggle functionality
						...
						?>
						<div class="custom-section-content custom-section-collapsed" id="custom-section-content-<?php echo esc_attr( $current_section->ID ); ?>" data-custom-section-content="<?php echo esc_attr( $current_section->ID ); ?>">
					<?php
				}
				
				// Display the lesson row (without section header since we handle it above)
				learndash_get_template_part(
					'lesson/partials/row.php',
					array(
						'count'                => $i,
						'sections'             => array(), // Don't show section header in row
						'lesson'               => $lesson,
						'course_id'            => $course_id,
						'user_id'              => $user_id,
			...
			if ( $current_section !== null ) {
				?>
				</div> <!-- Close last section lessons -->
			</div> <!-- Close last section wrapper -->
				<?php
			}
